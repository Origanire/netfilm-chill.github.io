...
def discover_movies(conn: sqlite3.Connection, pages: Optional[int] = None) -> List[dict]:
    ...
    for row in rows:
        ...
        if movie_id:
            ...
            movie["genre_ids"] = [r["genre_id"] for r in genre_rows]
        else:
            movie["genre_ids"] = []
...
class Question:
    ...
    def score(self, candidates: List[dict]) -> float:
        yes, no, unk = split_counts(candidates, self.predicate)
...
def choose_best_question(candidates: List[dict], questions: List[Question], asked: Set[str]) -> Optional[Question]:
    """
    Choisit la meilleure question de manière déterministe (plus d'aléatoire).
    Évite les contradictions et limite les jokers de titre.
    Gère les exclusions logiques (ex: si langue=anglais, exclure les autres langues)
    """
    contradictions = {
    ...
    # MODIFICATION: Toujours choisir la question avec le meilleur score (pas d'aléatoire)
    scored.sort(key=lambda x: x[1], reverse=True)
    return scored[0][0]  # Retourner directement la meilleure question
...
def should_enter_guess_mode(state: EngineState) -> bool:
    # déclenchement quand le pool devient petit ou quand le top domine nettement
    if len(state.candidates) <= 15:  # Réduit de 30 à 15
        return True
    if len(state.candidates) >= 2:
        s1 = score_of(state, state.candidates[0])
        s2 = score_of(state, state.candidates[1])
        if (s1 - s2) >= 2.0:  # Réduit de 3.0 à 2.0
            return True
    return False
...
def main() -> int:
    ...
    print("╔════════════════════════════════════════════════════════╗")
    print("║           AKINATOR DE FILMS (SQLite) - AMÉLIORÉ       ║")
    print("╚════════════════════════════════════════════════════════╝")
    print()
    ...
    try:
        ...
        while True:
            ...
            # ajoute des questions dynamiques basées sur le top actuel
            ...
            # priorité: keywords -> personnes -> statiques
            merged_questions = dyn_kw + dyn_people + questions

            q = choose_best_question(state.candidates, merged_questions, state.asked)

            # Convergence: si plus de question utile OU si le top domine, on propose un guess.
            # NOUVEAU: deviner plus agressivement quand il reste peu de candidats
            should_guess = False
            
            # Si moins de 100 candidats et le top domine (score >= 2 pts d'avance)
            if len(state.candidates) <= 100 and len(state.candidates) >= 2:
                s1 = score_of(state, state.candidates[0])
                s2 = score_of(state, state.candidates[1])
                if (s1 - s2) >= 2.0:
                    should_guess = True
            
            # Si on entre en mode guess normal OU si condition spéciale
            if q is None or (should_enter_guess_mode(state) and state.guess_cooldown == 0) or (should_guess and state.guess_cooldown == 0):
            ...
            state.question_count += 1
            state.consecutive_guesses = 0  # NOUVEAU: reset après une vraie question
